...
class WPLinkedInMUConnection extends WPLinkedInConnection {
	...
	private function get_or_create_user($profile) {
		...
		if (count($user_by_id) == 1) {
			...
			if (email_exists($email)) {
			    ...
			    $this->update_user_profile($user_id, $profile, true);
			    return $user_id;
			}
		}
	...
	private function update_user_profile($user_id, $profile, $new_user=false) {
		...
		if ($new_user) $user_info['role'] = get_option('wp-linkedin-mu_default_user_role', 'subscriber');
