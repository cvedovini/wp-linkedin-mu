...
class WPLinkedInCoAdmin {
	...
	function admin_notices() {
		if (!function_exists('wp_linkedin_connection')) {
			...
			echo '</li></ul></div>';
		} elseif (version_compare(WP_LINKEDIN_VERSION, '2.0') < 0) {
			$format = __('The WP LinkedIn for Company plugin requires at least version %s of the WP-LinkedIn plugin, current installed version is %s', 'wp-linkedin-mu');
			$error = sprintf($format, '2.0', WP_LINKEDIN_VERSION);
			echo '<div class="error" style="font-weight:bold;"><ul><li>';
			echo $error;
			echo '</li></ul></div>';
		}
