...
define('WP_LINKEDIN_MU_VERSION', '1.4');
include 'updater.php';
...
function wp_linkedin_mu_init() {
	...
	add_filter('linkedin_scope', 'wp_linkedin_mu_linkedin_scope');
	...
	if (get_option('wp-linkedin-mu_connect_with_linkedin')) {
		add_action('login_form', 'wp_linkedin_mu_show_login_button');
	}
...
function wp_linkedin_mu_admin_menu() {
	require_once 'class-admin.php';
	$admin = new WPLinkedInMUAdmin();
}
add_action('admin_menu', 'wp_linkedin_mu_admin_menu', 20);


function wp_linkedin_mu_linkedin_scope($scope) {
...
function wp_linkedin_mu_login_button($atts=array()) {
	...
	if (!is_user_logged_in()) {
		$atts = shortcode_atts(array(
				'button_label' => __('Connect with LinkedIn', 'wp-linkedin-mu'),
				'before_error' => '<p class="error">',
				'after_error' => '</p>',
				'before_success' => '<p class="info">',
				'after_success' => '</p>',
...
function wp_linkedin_mu_show_login_button() {
	...
	echo '<p style="margin-bottom:16px"><a href="' . $login_url . '">' . __('Connect with LinkedIn', 'wp-linkedin-mu') . '</a></p>';
}
